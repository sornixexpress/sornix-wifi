    ...
    try {
      ...
      if (path === "/router/bootstrap") {
        ...
        const rsc = [
          ...
          '# pull the captive-portal pages from the Worker (3 attempts each)',
          ':foreach f in={login.html;alogin.html;error.html;logout.html;redirect.html;status.html;sx.css} do={',
          '  :local ok false',
          '  :for try from=1 to=3 do={',
          '    :if ($ok = false) do={',
          '      :do { /tool fetch url=("$sxApi/router/files/" . $f . "?token=" . $sxTok) dst-path=("hotspot/" . $f) as-value; :set ok true } on-error={ :log warning ("sx-bootstrap: retry " . $f); :delay 2s }',
          '    }',
          '  }',
          '  :if ($ok = false) do={ :log error ("sx-bootstrap: FAILED to download " . $f) } else={ :log info ("sx-bootstrap: got " . $f) }',
          '}',
          '# install the sync script from a freshly fetched file (dedicated name = no stale file)',
          ':local ok2 false',
          ':for try from=1 to=3 do={',
          '  :if ($ok2 = false) do={',
          '    :do { /tool fetch url=("$sxApi/router/files/sx-sync.rsc?token=" . $sxTok) dst-path="sx-sync-new.rsc" as-value; :set ok2 true } on-error={ :log warning "sx-bootstrap: retry sx-sync"; :delay 2s }',
          '  }',
          '}',
          ':if ($ok2 = false) do={ :log error "sx-bootstrap: FAILED sx-sync.rsc - router keeps old sync script" } else={',
          ...
          '  :log info "sx-bootstrap: sx-sync script installed"',
          '}',
